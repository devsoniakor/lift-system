...
public class Building {
    ...
    private int top = 6;
    private IntegerProperty timeObs = new SimpleIntegerProperty(0);
    private int  time = 0;
    ...
    private LinkedList<BuildingObserver> observers = new LinkedList<BuildingObserver>();
    ...
    public Building() {
        ...
        observers.add(new TimeObserver());
        timeObs.set(time);
        
    }
    
    //Inner Class observer: BuildingObserver interface implemented
    private class TimeObserver implements BuildingObserver {
        @Override public void handleTime(int newTime){
            timeObs.set(newTime);
        }
    }
            ...
            while (running) {
                try {
                    ...
                    Platform.runLater(() -> {
                        operate();
                        time++;
                        for(BuildingObserver o : observers)
                            o.handleTime(time);
